...
2. **Botón de pausa en la esquina superior derecha** (#94/Q23a), dentro de `.world-view__header`, junto al HUD. Siempre accesible mientras no haya un modal abierto (#85). Tecla Escape NO lo abre: Escape ya sale de un nivel de cámara por pulsación en la vista de mundo y robarle ese gesto rompería lo cerrado en 4c.1.
...
## Estructura interna requerida
1. **Menú principal = ramas `empty` y `fallen` de home-view**, sin cambios estructurales. La lápida se queda aquí (#94/Q13): ponerla dentro del POI Hogar la convertiría en herencia entre runs, contra #94 y C3b de #85.
2. **La rama `alive` cambia de sentido**: deja de ser "estás en casa, sal al yermo" y pasa a ser **Cargar partida**. Botón primario "Cargar partida" → mundo restaurado en la vista persistida (#90, Q12c). El PJ que aún no ha hecho el tutorial del Lobo (`tutorial_lobo_completed === false`) sigue viendo además la vía del tutorial, que no se toca (#86): saltárselo cuesta loot y XP, y esa elección se conserva.
...
4. **El Hogar se lee como sitio, no como POI genérico**: nombre "El Hogar" (ya lo devuelve `poiDisplayName`), y un texto de escena propio, distinto del placeholder idéntico de los otros 719. Es el único POI con contenido real en 4c porque es el único que ya tiene función mecánica. Una frase, tono #47 (post-humano, naturaleza vencedora), sin narrativa final inventada.
...
- No queda ninguna vía que lleve a la pantalla home estando vivo dentro de la run.
