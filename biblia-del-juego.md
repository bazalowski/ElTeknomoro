...
**Muerte por fatiga.** Cuando `hp.max ≤ 0` al despertar → fin de run con `last_damage_source='fatigue'` (#85) y epitafio normal. Con `computeMaxHp = 8 + 2·CON` la inanición mata en **3-4 noches**: la agonía es la propia acumulación, no un contador aparte. Un PJ famélico que cae en combate sale con `'enemy'` — `last_damage_source` registra la causa próxima, no la contribuyente (#98).
