...
| El efecto de `pocion_curacion_menor` está declarado en `data/items.ts` pero sin cablear. Desde el matiz de #98 ya no es la única vía de curarse: acampar con ración cura. | #98 | H6 |
...
| 103 | **Qué es un ancla y cuándo un grid está Controlado (cerrado en cuestionario de scope 4e, arbitraje delegado).** El cuestionario definía el ancla de cuatro maneras incompatibles a la vez: Q1b la hacía marca lógica automática y sin coste, Q12c un modal que pregunta, Q13 "son un item" y Q15a algo que se recoge y se mueve. Se resuelve a favor de **objeto físico** (Q1a), la única lectura compatible con Q13, Q15a, Q16b y con la letra de #70 ("colocadas explícitamente por el jugador"); Q1b además reabría una decisión que la sección A del propio cuestionario declara no reabrible. **Plantar cuesta 1 acción** y consume el item — la rama del default de Q14 que aplica cuando el ancla es física. El juego **propone** plantarla con modal `[Sí]`/`[No]` al alcanzar Controlado (Q12c) y nunca lo hace solo. **Cap por nivel: 3 anclas, +1 cada 3 niveles, tope 6** (Q2b; la curva no estaba en el cuestionario y Q16 la mezclaba con el cap fijo de 5 de Q2c). **El ancla sale de la fórmula de Controlado.** Con Q1b y `deriveGridState` como estaba, Controlado dependía del ancla y el ancla de Controlado — circularidad pura; y Q15a habría devuelto un grid Controlado a Explorado, contra Q11. **Controlado = los 4 POIs del grid visitados** (Q7a), sin ancla en la ecuación y **sin retroceso** (Q11); el ancla es un permiso de viaje que se planta encima de un grid que ya lo es. **Consecuencia de orden: 4e deja de depender de 4f.** El código pedía `completado` y §3 tenía escrita esa dependencia, pero `revelado` ya lo escribe 4c al entrar a un POI, así que con Q7a la condición es alcanzable hoy y 4e pasa a ser el siguiente sub-paso. El argumento de #97 sigue intacto —4f es el sub-paso grande y el que abre la escritura de contenido—; lo que cambia es que 4e ya no espera a nadie. **Sin anclas prediseñadas** (Q5a, contra Q5b): §9.8 decía literalmente "no nodos prediseñados", el dataset de 4a tiene **74 grids con POI Asentamiento sobre 180**, y regalarlas todas es el fast travel gratuito que el propio Q41 declara el trope a evitar, además de dejar sin sentido el cap de Q2 y el bloqueo de Q16. **Única excepción: el Hogar** nace con ancla (Q6). Ojo con Q42 ("anclas = hogueras de Dark Souls"): describe el modelo de Q5b y no el de #70, porque las hogueras son fijas y prediseñadas. **El ancla cubre el grid entero** (Q4a, contra Q4c): con Q24 y Q44, una posición libre dentro del grid no cambiaba ninguna mecánica y sólo añadía un paso de UX sin consecuencia. **Las anclas mueren con el PJ** (Q3a): `worldState.anchors` cuelga del slot (#90) y se rebobina con la muerte (#94). **Reescribe**: §9.6 punto 2, §9.8. **Matiza**: la línea de dependencias de §3 y el orden de #97. | v0.32 |
| 104 | **UX y coste del fast travel (cerrado en cuestionario de scope 4e, arbitraje delegado).** **El viaje no se inicia desde el POI Asentamiento**: Q17c se descarta y se adopta Q17d. El reparto de arquetipos de 4b.0 deja **74 grids de 180 con POI Asentamiento**, y sólo 9 de 30 en el Este y otros 9 de 30 en el Oeste, así que el botón de Q17c no existiría en 106 grids y Q24 (llegar al POI Asentamiento del destino) sería imposible en esos mismos 106. Q17c choca además con Q39, que exige salir del POI para viajar. Se adopta **vista regional + botón de HUD**, que es lo que Q18 (icono sobre el grid) y Q19b ("distancia desde grid actual") ya daban por hecho, y **llegada a posición neutral** del grid. **Distancia en Manhattan** (Q26a, contra Q26c): Q25b exige una distancia que ya calcule y Q26c la mandaba a un sistema de rutas que no existe —4a es cartesiano y #88 fija adyacencia cardinal—; la firma queda preparada para que un sistema de rutas la sustituya sin tocar callers, el mismo patrón que #83 exige. **Coste `1 ración + (1 + ceil(d/5))` acciones, con clamp [2, 5]** (Q25b + Q27). Esto **matiza #83**, que dice "valor plano hoy, proporcional mañana": entra proporcional desde el primer día, y queda escrito como matiz y no como lectura. Sobre el bounding box de 22×15 la curva da 2 acciones hasta distancia 5 y 5 a partir de 16. **Sin límite de viajes por día** (Q47 descartada): el motivo que lo pedía —"que el contador de acciones no sea irrelevante"— ya lo cubre el coste, que se come hasta 5 de las 8 acciones; un tope duro encima aplana la curva de distancia hasta hacerla invisible y mata la decisión estratégica que Q41 pide, y nada en §9.8 ni en #83 lo respaldaba. **Resto de UX**: coste visible antes de confirmar (Q20a); sin recursos, botón deshabilitado con copy que dice qué falta **y** cómo conseguirlo (Q21, la mezcla de a y b que pedía la respuesta); prohibido viajar al grid propio (Q22) y desde dentro de un POI (Q39); **fade de 200-300 ms** (Q23b) respetando `prefers-reduced-motion` como #91; icono de ancla con color propio sobre el grid en vista regional, **sutil pero inequívoco** (Q18, Q46). **Viaje siempre seguro en 4e** (Q37): sin tirada de riesgo, sin modificador por modo Libre (Q28) ni por perks o atributos (Q29), y sin sabor narrativo, que es contenido de fase 2 (Q43). El tramo arriesgado con tirada condensada del concepto #21, que Q50 anticipa como "1d20 si viajas más de X grids", queda diferido a 4f/H5. **El "diario de viaje" de Q49 no es sistema nuevo**: no existe en la biblia, y se lee como que el viaje aparece en el resumen del día del modal de acampar (4d). **Reescribe**: §9.8. **Matiza**: #83. | v0.32 |
| 105 | **`fast-travel.ts` se reescribe entero y `deriveGridState` cambia de fórmula (cerrado en cuestionario de scope 4e, Bloque F).** Q31 pedía "cablear el modelo viejo y mejorar las reglas sobre lo ya creado". No hay modelo viejo que cablear: `src/rules/fast-travel.ts` corre sobre `WorldMap`, `NodeId` y `WorldEdge` de `world-gen.ts` —un grafo procedural— y sobre `rollExplorationTick` de `exploration.ts` —tablas por bioma—, y **ninguna de las dos cosas existe ya**: el mundo son 180 grids fijos (#72) y las tablas son por POI (#92, #102). El dataset de 4a no contiene un solo `WorldEdge`. Sus únicos consumidores son tres smoke tests del esqueleto, que se retiran con él. Se **reescribe el módulo entero** conservando los nombres del cuestionario: `canFastTravelTo`, `computeFastTravelCost`, `executeFastTravel`. Lo único que se hereda del modelo viejo es la idea de tramo arriesgado con tirada condensada, que es Q50 y va a 4f/H5. **Arquitectura** (Q30c): `rules/fast-travel.ts` SAGRADO y puro decide, `state/` orquesta y persiste. Misma frontera que #94 puso para los POIs, #100 para la jornada y #58 para el loot. **`Character` no se toca** (Q33): `worldState.anchors` existe desde 4b.0 con el comentario literal "los cablea 4e", y es además lo único que cumple Q3a, porque el slot se rebobina con la muerte. **Controlado sigue derivado y no persistido** (Q32b), pero cambia la fórmula: `deriveGridState` pedía `completed === total && hasAnchor` y pasa a pedir `revealed === total` (#103). El ancla sale de la derivación. **Tests: 20-25 unitarios**, cubriendo el cap por nivel, recoger y replantar, el clamp del coste, los edge cases de Q35, Q38 y Q40, y la regresión de que Controlado no retrocede. **Edge cases resueltos por precedente**: un click durante una animación la cancela y arranca la nueva, nunca se encola (Q36, ya cerrado en #91 y #93); agotar la jornada viajando cae en la misma lógica de 4d, verbos deshabilitados y acampar a un click (Q38, #99); cerrar el navegador a mitad de modal deja al PJ en el grid origen sin cobrar nada (Q40), porque el cobro y el movimiento son la misma transacción pura. | v0.32 |
...
2. **Estado del grid:** Inexplorado (todos los POIs ???) / Explorado (POIs visibles, tablas activas) / Controlado (fast travel disponible, recursos garantizados, tablas con menor varianza). **Cuándo se alcanza cada uno lo fija #103:** Controlado son los 4 POIs del grid **visitados** —no completados—, sin el ancla en la fórmula y sin retroceso posible.
...
- **Con ración:** consume 1 ración, resetea las 8 acciones, incrementa el día y **cura hasta el `hp.max` vigente**. Si la inanición ya degradó el máximo, se recupera hasta el techo que quedó, no hasta el original (Q33 sigue postergada).
...
### 9.8 Fast travel y anclas (decisiones #70, #83, #103, #104, #105)

**Estado: cerrado en v0.32.** El marco lo abrió #70 y el coste #83; el cuestionario de scope de 4e lo cerró operativamente en #103 (qué es un ancla y qué es un grid Controlado), #104 (UX y coste del viaje) y #105 (cableado).

**Grid Controlado.** Un grid pasa a Controlado cuando sus **4 POIs han sido visitados** — visitados, no completados. El estado **no retrocede nunca**. El ancla no entra en la fórmula: Controlado es una condición del grid, y el ancla es un permiso de viaje que se planta encima.

**Ancla.** **Objeto físico** del inventario que el jugador planta en un grid Controlado. Instalarla cuesta **1 acción** y consume el item. Al alcanzar Controlado el juego **propone** plantarla con un modal `[Sí]` / `[No]`; nunca se planta sola. Se puede **recoger y replantar** en otro grid: recoger devuelve el item y no toca el estado del grid. **Cap por nivel del PJ: 3 anclas, +1 cada 3 niveles, tope 6.** Al alcanzar el cap se bloquea con copy explicativo — ni sustitución automática ni modal de elección.

**No hay anclas prediseñadas.** Una sola excepción: el **Hogar** (POI Asentamiento de `sur-001`) nace con ancla, porque es el punto de aparición y el hub. Los POIs Asentamiento del dataset darían **74 anclas gratis sobre 180 grids**, que es otro sistema y no una excepción.

**Las anclas mueren con el PJ.** Viven en `worldState.anchors`, que cuelga del slot (#90) y se rebobina entero con la muerte (#94). Permadeath puro, coherente con C3b de #85.

**Coste del viaje.** `1 ración + (1 + ceil(distancia / 5))` acciones, con **clamp entre 2 y 5 acciones**. La distancia es **Manhattan** entre grid origen y destino sobre el cartesiano de 4a. **No hay límite de viajes por día:** el coste ya es el freno, y un tope duro encima aplanaría la curva de distancia hasta hacerla invisible.

**El viaje.** Se inicia desde la **vista regional** (seleccionar grid con ancla → botón de confirmación, la separación de #88) o desde un **botón de HUD** que abre la lista de anclas con nombre, región, distancia y coste. El coste se muestra **antes** de confirmar. Sin recursos suficientes el botón queda deshabilitado con copy que dice qué falta **y** cómo conseguirlo. No se viaja al grid donde ya está el PJ, ni desde dentro de un POI: hay que salir antes. Transición de **fade corto (200-300 ms)**, respetando `prefers-reduced-motion` como #91. El PJ llega a una **posición neutral** del grid de destino. En vista regional, un grid con ancla se distingue por icono y color propio: sutil pero inequívoco.

**Lo que 4e no hace.** El estado Controlado sólo habilita fast travel; sus efectos sobre los pesos de la tabla d20 (§9.6) entran con 4f. El viaje es **siempre seguro**: no hay tirada de riesgo, ni modulación por modo Libre (#13) ni por perks. El tramo arriesgado con tirada condensada (concepto #21) queda diferido a 4f/H5, y el sabor narrativo del viaje es contenido de fase 2.
...
| 4e | **Scope cerrado en v0.32 (#103-#105). Adelantado a 4f por #103.** Fast travel + anclas + condición de Controlado. Ancla como item físico con cap por nivel, propuesta por modal al Controlar el grid, recogible y replantable. Viaje desde vista regional y botón de HUD; coste `1 ración + (1 + ceil(d/5))` acciones con clamp [2, 5] sobre distancia Manhattan. `src/rules/fast-travel.ts` se reescribe entero y `deriveGridState` cambia de fórmula. **No entra**: tirada de riesgo en el trayecto (4f/H5), sabor narrativo del viaje (fase 2), ni efectos de Controlado sobre la tabla d20 (4f). | SÍ |
...
**Dos ajustes de cierre de 4d (26/8/2026, tras la sesión de prueba de Bazalo).**

- **Acampar con ración cura.** Ver el matiz de #98 arriba: la lectura original extendía Q12b más allá de lo que decía.
- **Marcador del PJ en la vista regional, más sutil.** El anillo tenía radio 2,6 sobre una celda de 10, o sea el **52% del grid**, con un punto relleno de otro 30%. Sobre las regiones cálidas del dataset (`#8B7355` del Centro, `#A87B5C` del Este) el conjunto anillo-hueso + punto-hueso se leía como una chapa dorada pegada encima del mapa. Anillo a 1,5 (30%) y punto a 0,5 (10%), con el trazo más fino: sigue diciendo "estás aquí" sin tapar el grid que señala.

**Estado al cierre de v0.31:**

- Biblia: 13 secciones, 102 decisiones numeradas, 0 bloqueantes abiertos.
- Código: **660/660 tests** (+12), `tsc --noEmit` y `vite build` limpios.
- H4: 6 de 8 sub-pasos cerrados. Quedan **4f** y **4e**.


---

**v0.32** — **Cierre de scope del sub-paso 4e** (27/8/2026). Tres decisiones nuevas (#103, #104, #105) desde el cuestionario de scope de 4e, respondido por Bazalo y arbitrado por delegación.

El cuestionario llegó con **once contradicciones**, nueve de ellas bloqueantes. Las cuatro que cambiaban el sub-paso entero:

- **El ancla se definía de cuatro maneras a la vez.** Q1b la hacía marca lógica automática y sin coste; Q13 la llamaba item, Q15a permitía recogerla y Q12c pedía un modal que pregunte. Sólo "objeto físico" sobrevive al cruce, y es además lo que #70 llevaba escrito desde v0.20.
- **Controlado y el ancla se definían el uno al otro.** `deriveGridState` pedía ancla para declarar un grid Controlado y Q1b plantaba el ancla al alcanzar Controlado. El ancla sale de la fórmula: Controlado son los 4 POIs visitados y nada más.
- **Ese mismo cambio desbloquea el sub-paso.** El código pedía POIs `completado`, que es deuda de 4f, y por eso §3 tenía escrito "4e depende de 4f". Con Controlado medido en POIs **visitados** —y `revelado` lo escribe 4c al entrar— la condición es alcanzable hoy. 4e se adelanta a 4f sin deuda; el argumento de #97 para el orden anterior no se toca, simplemente deja de aplicar a 4e.
- **Q17c era imposible en 106 de los 180 grids.** Poner el botón de viaje dentro del POI Asentamiento parecía diegético hasta contar el dataset: 74 grids tienen uno. El Este y el Oeste tienen 9 cada uno sobre 30 grids. Y Q5b, que quería anclas prediseñadas en esos mismos POIs, regalaba 74 anclas gratis — el fast travel gratuito que Q41 nombra como el trope a evitar.

Dos correcciones más que no venían del cuestionario sino de cruzarlo con el repo: **Q26c medía distancias con un sistema de rutas que no existe** (se adopta Manhattan sobre el cartesiano de 4a), y **Q31 pedía cablear un módulo sin datos que cablear** — `fast-travel.ts` corre sobre un grafo procedural y unas tablas por bioma que el modelo vigente jubiló hace dos sub-pasos.

**Coste por distancia desde el día uno.** #83 decía "plano hoy, proporcional mañana" y aquí entra proporcional ya: `1 + ceil(d/5)` acciones con clamp [2, 5]. Queda registrado como matiz explícito a #83, no como lectura suya. A cambio cae el tope de 1 viaje por día de Q47: el coste ya se come hasta 5 de las 8 acciones, y un tope duro encima haría que todos los viajes costasen lo mismo en la práctica.

**Cabecera del documento corregida.** Decía v0.26 desde el bump de v0.27: cinco versiones desincronizada con §3.

**Cambios estructurales:**

- §3: estado, "Próximo" y mapa de H4 al día. 4e y 4f intercambian fila y estado. La línea de dependencias se reescribe: de "4e depende de los otros dos" a "4e ya no depende de 4f".
- §5: 3 decisiones nuevas (#103-#105). 105 decisiones cerradas (antes 102).
- §9.6 punto 2: gana la condición de Controlado, que hasta ahora no estaba escrita en ningún sitio.
- §9.8 reescrita entera, de cuatro viñetas a sección cerrada. Cambia de título a "Fast travel y anclas".
- §13: fila de 4e con el scope completo.
- Sin cambios en §1, §2, §4, §6 (siguen 0 bloqueantes), §7, §8, §10, §11, §12.

**Estado al cierre de v0.32:**
