...
> **ESTADO: CERRADO** (6/5/2026). Respondido por Bazalo y procesado por el director. 7 fricciones levantadas, 5 bloqueantes resueltas, 10 preguntas delegadas al director. Cierre volcado en **biblia v0.23**: decisiones #86-#91, matices en #45 y #84, sub-paso **4b.0** nuevo. MODOPIPELINE autorizado para 4b tras 4b.0.
>
...
1. [★] Hoy el flow es: home → botón "Entrar al yermo" → combate Lobo → modal de loot/epitafio → home. Tras 4b, ¿cómo se llega a la vista regional la primera vez?
   ...
   X - (b) Tras el combate Lobo (cuando `tutorial_lobo_completed=true`), el botón "Entrar al yermo" se sustituye por "Salir al mundo" → vista regional. La home sigue siendo pantalla aparte hasta que 4c reinterprete home como POI. Se crea una ventana para crear un nuevo personaje con el tutorial activo. 
...
- Actuaria como un menú de pausa. Puedes acceder al inventario y equipo, descansar, abrir los ajustes, cambiar de personaje o iniciar el mundo.
...
- A) Por ahora lo mas facil. EL diseño lo iremos cambiando a futuro.
...
- Etiquetas geográficas muy leves.
...
- Pinta algun elemento decorativo, pero sutil, en concordancia al lore.
...
- Mantengamos la vision actual. Esto se ajustara despues.
...
- Puedes hacer click en cualquier grid, pero solo moverte a los que te permita la mecanica. La informacion solo se descubre si ya has explorado el grid.
...
- Si. Seria como salir del juego al menu principal.
...
- Todo POI tiene su icono diferente.
...
- Hay fondo de grid. La idea es que los fondos esten diseñados a mano, pero es algo visual no influyente actualmente.
...
- Se podria meter una libreria de animacion, si la tarea no requiere una gran carga
...
- Se refiere a todo. Sirve como un "guardar partida".
...
- Lo que sientas mas adecuado. Creo que un save ligado al user que juega es la mejor opcion.
...
- Si. EL tutorial solo es obligatorio la primera vez. UNa vez completado, a ese personaje en cuestion se le añade un badge que permita jugar el tutorial o no.
...
- Una mezcla de Slay of Spire con las cuadriculas de Project Zomboid.
